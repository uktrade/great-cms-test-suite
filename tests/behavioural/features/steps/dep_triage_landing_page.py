...
def step_visit_homepage(context):
    ...
    assert homepage.context.browser.current_url.rstrip('/') == homepage.url.rstrip('/')
...
def step_load_dep_triage_landing_page(context):
    ...
    assert guidance_and_support_page.context.browser.current_url.rstrip('/') == guidance_and_support_page.url.rstrip(
        '/'
    )
